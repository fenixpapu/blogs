...
```javascript
  const getUserInfo = ms => new Promise(res => {
    setTimeout(()=> res('userInfo'), ms);
  });
  const getBalance = ms => new Promise(res => {
    setTimeout(() => res('balance'), ms);
  });
  const getTransaction = ms => new Promise(res => {
...
- Anh minh hoa luong thoi gian cua cac Promise o day.
